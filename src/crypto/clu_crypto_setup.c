...
#endif
#if defined(WOLFSSL_AES_COUNTER) && LIBWOLFSSL_VERSION_HEX >= 0x05009000
...
static void wolfCLU_encryptHelp(void)
{
    ...
    WOLFCLU_LOG(WOLFCLU_L0,
                "\t-pass option for password source i.e pass:<password>");
...
int wolfCLU_setup(int argc, char **argv, char action)
{
    ...
    pwdKey =
        (byte *)XMALLOC(keySize + block, HEAP_HINT, DYNAMIC_TYPE_TMP_BUFFER);
    if (pwdKey == NULL) {
    ...
    while ((option = wolfCLU_GetOpt(argc, argv, "", crypt_options,
        ...
        switch (option) {
            ...
            case WOLFCLU_KEY: /* hex key string from the command line */
                ...
                ret = wolfCLU_loadHexKeyInto(key, (keySize + 7) / 8, optarg,
                                             (word32)XSTRLEN(optarg));
                if (ret != WOLFCLU_SUCCESS) {
                    wolfCLU_freeBins(pwdKey, iv, key, (byte *)mode, NULL);
                    return ret;
                }
            ...
            case WOLFCLU_IV: /* IV if used must be in hex */
            {
                ...
                ret =
                    wolfCLU_hexToBin(ivString, &ivTmp, &ivTmpSz, NULL, NULL,
                                     NULL, NULL, NULL, NULL, NULL, NULL, NULL);
                XFREE(ivString, HEAP_HINT, DYNAMIC_TYPE_TMP_BUFFER);
                if (ret != WOLFCLU_SUCCESS) {
                    WOLFCLU_LOG(WOLFCLU_E0,
                                "failed during conversion of IV, ret = %d",
                                ret);
                    wolfCLU_freeBins(pwdKey, iv, key, (byte *)mode, NULL);
                    return WOLFCLU_FATAL_ERROR;
                }
            ...
            case WOLFCLU_INKEY: {
                ...
                if (isHex) {
                    ...
                    ret = wolfCLU_loadHexKeyInto(key, keyBytes, keyString,
                                                 (word32)j);
    ...
    if (encCheck == 1) {
        ...
        if (cphr != NULL) {
            ret = wolfCLU_evp_crypto(
                cphr, mode, pwdKey, key, (keySize + 7) / 8, in, out, NULL, iv,
                0, 1, pbkVersion, hashType, verbose, isBase64, noSalt, keyType);
        }
        else {
            if (outCheck == 0) {
                ret = wolfCLU_readFilename(
                    outNameEnc, sizeof(outNameEnc),
            ...
            ret = wolfCLU_encrypt(alg, mode, pwdKey, key, keySize, in, out, iv,
                                  block, ivCheck, inputHex);
        }
    }
    /* decryption function call */
        ...
        if (cphr != NULL) {
            ret = wolfCLU_evp_crypto(
                cphr, mode, pwdKey, key, (keySize + 7) / 8, in, out, NULL, iv,
                0, 0, pbkVersion, hashType, verbose, isBase64, noSalt, keyType);
        }
        else {
            if (outCheck == 0) {
                ret = wolfCLU_readFilename(
                    outNameDec, sizeof(outNameDec),
                    "Please enter a name for the output file: ");
                if (ret != WOLFCLU_SUCCESS) {
                    wolfCLU_freeBins(pwdKey, iv, key, (byte *)mode, NULL);
                    return WOLFCLU_FATAL_ERROR;
            ...
            ret = wolfCLU_decrypt(alg, mode, pwdKey, key, keySize, in, out, iv,
                                  block, keyType);
        }
    }
    else {
